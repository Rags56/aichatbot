...
def get_debug_info():
    """Get debug information about the AI system"""
    try:
        if not ai_service:
            return jsonify({
        ...
        debug_info = ai_service.get_ollama_system_info()
        
        # Add additional GPU diagnostics
        try:
            import subprocess
            # Check nvidia-smi for GPU utilization
            nvidia_result = subprocess.run(['nvidia-smi', '--query-gpu=name,utilization.gpu,memory.used,memory.total', '--format=csv,noheader,nounits'], 
            ...
            if nvidia_result.returncode == 0 and nvidia_result.stdout.strip():
                ...
                if gpu_lines:
                    ...
                    debug_info['gpu_utilization'] = gpu_data[1].strip() if len(gpu_data) > 1 else 'N/A'
                    debug_info['gpu_memory'] = f"{gpu_data[2].strip()}/{gpu_data[3].strip()} MB" if len(gpu_data) > 3 else 'N/A'
        except:
            pass
...
def _execute_query(user_id: str, query_text: str, filters: dict = None, top_k: int = 6, 
    ...
    try:
        ...
        # Preserve existing meta (including pipeline_timing) and add new fields
