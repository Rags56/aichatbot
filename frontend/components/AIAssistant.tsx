...
import { aiApi, checkBackendHealth } from '@/lib/api'
    ...
    const checkHealth = async () => {
      ...
      if (!isOnline) {
        toast({
          ...
          description: 'Cannot connect to backend. Please check your connection.',
        })
      }
    }
    checkHealth()
  ...
  const handleSend = async () => {
    ...
    if (!isOnline) {
      ...
      toast({
        variant: 'destructive',
        title: 'Backend Offline',
        description: 'Cannot send message. Backend is not available. Please try again.',
      })
      return
    }
    ...
    try {
      ...
      const assistantMessage: Message = {
        ...
        meta: {
          ...response.data.meta || {},
          pipeline_timing: response.data.meta?.pipeline_timing || {}
        }
      }
      ...
      // Store pipeline timing for debug panel
      if (response.data.meta?.pipeline_timing) {
        sessionStorage.setItem('lastPipelineTiming', JSON.stringify(response.data.meta.pipeline_timing))
      }
    } catch (error: any) {
        ...
        <CardContent className="flex-1 flex flex-col pt-6">
          ...
          <div className="flex gap-2">
            <Input
              ...
              placeholder={backendOnline ? "Type your message..." : "Backend offline - cannot send messages"}
              disabled={loading || !backendOnline}
              className={`border-gray-300 focus:border-blue-500 focus:ring-blue-500 ${!backendOnline ? 'bg-gray-100' : ''}`}
            />
            <Button 
              onClick={handleSend} 
              ...
              title={!backendOnline ? 'Backend is offline' : ''}
            >
